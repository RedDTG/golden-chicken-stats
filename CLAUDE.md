...
`on_ready` syncs both globally (`bot.tree.sync()`, so newly-joined guilds pick commands up too) and per-guild (`copy_global_to()` + `sync(guild=...)` for every guild already in `bot.guilds`) — a global-only sync can take up to an hour to actually show up on Discord clients after adding/changing a command, while a guild sync is near-instant, which matters a lot during iteration. Sync failures are logged instead of failing silently.
